...
This tileset (knots) was taken from the original WFC github*/
...
let collapsed = [];
let uncollapsed = [];
const cellSize = 30; //Cells are always 30x30 pixels.
...
function preload() { //Load all tile images into an array
  ...
  tileImages.push(loadImage("WaveFunctionCollapse/images/About/empty.png"));
  tileImages.push(loadImage("WaveFunctionCollapse/images/About/cross.png"));
  tileImages.push(loadImage("WaveFunctionCollapse/images/About/line.png"));
  tileImages.push(loadImage("WaveFunctionCollapse/images/About/t.png"));
}
...
function setup() {
  ...
  tiles.push(new Tile(tileImages[0], ["1", "1", "0", "0"]));
  tiles.push(new Tile(tileImages[1], ["0", "0", "0", "0"]));
  tiles.push(new Tile(tileImages[2], ["1", "1", "1", "1"]));
  tiles.push(new Tile(tileImages[3], ["0", "1", "0", "1"]));
  tiles.push(new Tile(tileImages[4], ["0", "1", "1", "1"]));

  //Adds every rotated tile
  const stop = tiles.length - 1;
